    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
       ...
       if(StringUtils.isNotEmpty(referer)) {
           try {
               ...
               logger.info("host-------------"+parseDomainName.getServerIP().getHostAddress());
               if (!StringUtils.equals(HOST, parseDomainName.getServerIP().getHostAddress())) {
                   return checkSign(response, "用户登录异常，请重新登录");
               }
...
*/
        ...
        if (method.isAnnotationPresent(CheckToken.class)) {
            ...
            *//*if (current - timestamp > 30000) {
            ...
            for (Map.Entry entry : list) {
                if ("userId".equals(entry.getKey())||"companyUUid".equals(entry.getKey())||"json".equals(entry.getKey())
                        ||"timestamp".equals(entry.getKey()) || "sign".equals(entry.getKey())) {
                    continue;
                }
    ...
    public  boolean checkToken(HttpServletResponse httpServletResponse,String msg){
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("return_code", ReturnCode.FAIL);
        jsonObject.put("return_msg",msg);
        httpServletResponse.setCharacterEncoding("UTF-8");
        httpServletResponse.setContentType("application/json; charset=utf-8");
        try {
